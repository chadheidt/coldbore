...
APP_VERSION = "0.14.10"  # keep in sync with app/version.py
...
# v0.14.5 Path B: pre-rendered demo screenshots shown in the single-
# window demo (no Excel needed at runtime). Generated by
# tools/render_demo_screenshots.py. Placed in Contents/Resources/
# demo_screenshots/ — demo_tour.get_demo_screenshot() resolves them
# via sys.executable in the .app.
DEMO_SHOTS_DIR = HERE / "app" / "resources" / "demo_screenshots"
_shots = sorted(str(p) for p in DEMO_SHOTS_DIR.glob("*.png")) \
    if DEMO_SHOTS_DIR.is_dir() else []
if _shots:
    DATA_FILES.append(("demo_screenshots", _shots))
else:
