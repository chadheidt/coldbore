# Send True Zero to friends

Bookmark this file. Everything you need to share True Zero is here.
...
True Zero is in private beta — the website Download button is gated behind a unique access code. Each tester needs both **the link AND their personal access code** to install.

---

## Before you send: issue the tester a key

Every new tester needs a unique CBORE-XXXX-XXXX-XXXX-XXXX code. The full click-by-click for issuing a code is in `True Zero — How to Issue License Keys.docx` on your Desktop. Short version:
...
> Sending you True Zero — Mac app I built for tracking rifle load development. It's in private beta so you'll need this access code: `CBORE-XXXX-XXXX-XXXX-XXXX`. Click Download at https://chadheidt.github.io/coldbore/ and paste the code. After installing, open True Zero and paste the same code in the license dialog. The Quick Start docx is on the disk image. Don't share the code please — it's tied to you so I can follow up.

### Longer version (email):

> Hey,
>
> Want to send you something I've been working on. True Zero is a Mac app I built that takes Garmin Xero and BallisticX CSV files and auto-fills my load development workbook — so I don't have to type velocity / SD / group size / etc. by hand after every range trip.
>
> True Zero is in private beta. To install it, you'll need your personal access code:
>
> `CBORE-XXXX-XXXX-XXXX-XXXX`
>
> Steps:
> 1. Visit https://chadheidt.github.io/coldbore/
> 2. Click **Download True Zero** at the top of the page. A small window will pop up asking for your access code — paste the one above.
> 3. The True Zero disk image will download. Double-click it, drag **True Zero** to the **Applications** shortcut in the same window.
> 4. Open True Zero from your Applications folder. A license dialog will appear — paste the same access code again to unlock the app.
...
Just drag them onto the True Zero window (or onto the True Zero icon in the Dock). The app figures out whether each one is Garmin or BallisticX and routes it correctly.
...
Make sure Excel is closed before running the import — True Zero can't write to a workbook that's already open. Also make sure they're using their working .xlsx file (not the .xltx template).

**"How do I get a new version when you ship one?"**
The app checks automatically on launch. When a new version is out, a yellow banner appears at the top of the window. They click **Install Update** (the app downloads in the background using their license key for authentication), then **Quit and Install** when ready. True Zero quits, swaps itself, and reopens at the new version — no manual download or drag-to-Applications needed.

**"I lost my access code, can I get it back?"**
They email you. Their code is in `beta-keys.txt`.

**"Can I install True Zero on my second Mac?"**
