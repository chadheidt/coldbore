#!/bin/bash
# Build, code-sign, notarize, and DMG-package Loadscope for distribution.
#
...
#   1. Runs Build App.command logic to produce dist/Loadscope.app
...
#
# Output: dist/Loadscope.dmg  (signed, notarized, ready to ship)
#
...
APP_NAME="Loadscope"
DMG_NAME="Loadscope.dmg"
QUICKSTART="$PROJECT/Loadscope — Quick Start.docx"

# ----- Sanity checks ---------------------------------------------------------
echo "============================================================"
echo "Loadscope — building SIGNED + NOTARIZED .app + .dmg"
echo "============================================================"
echo ""
...
echo "[8/8] Building Loadscope.zip for v0.8.x auto-update path…"
ZIP_PATH="$BUILD_DIR/dist/Loadscope.zip"
rm -f "$ZIP_PATH"
cd "$BUILD_DIR/dist"
ditto -c -k --keepParent "Loadscope.app" "Loadscope.zip"
if [ -f "$QUICKSTART" ]; then
    cp "$QUICKSTART" .
    QS_BASENAME=$(basename "$QUICKSTART")
    zip -j "Loadscope.zip" "$QS_BASENAME"
    rm -f "$QS_BASENAME"
fi
cd "$PROJECT"
...
ZIP_PATH="$PROJECT/dist/Loadscope.zip"
