...
### Ballistic solver — STATUS (2026-05-16 NIGHT: GATE 1 ✅, GATE 3 ✅, GATE 2 17/61, GATE 4 approved+queued)
- **CURRENT (read this first):** Gate 1 ✅ done+signed off. Gate 3 ✅ done (commit 2557422: Chad-approved disclaimer clause #4 + DISCLAIMER_VERSION 1→2 + 4 guard tests; lawyer one-time EULA review flagged in [[loadscope-commerce-flip-on]] Step 0). Solver engine complete: **G7+G1 both authoritative + JBM-validated ≤0.07 mil**, 191 tests green, STILL not wired/shipped. Gate 2 (BC DB) IN PROGRESS 17/61. Gate 4 (predicted-vs-confirmed UX) **design approved by Chad — queued to build, with a pre-ship rendered preview**.
- **GATE 4 SCOPE NOW INCLUDES THE DEMO (Chad 2026-05-16 "add the solver to the demo"):** building the Range&DOPE predicted-vs-confirmed UX must also (a) render that REAL screen → new/updated Path-B demo screenshot, (b) replace the `TOUR_STOPS[3]` ⚠️PLACEHOLDER narration with the drafted predicted-DOPE script (in `Ballistic Solver — Gates 3 & 4 Review Draft.md`), (c) recommended: give the solver its OWN strong demo beat (predictions appear gray → a row confirmed black → Pocket Card w/ watermark) since it's the bigger pitch. Do NOT mock it before the UX exists (parity rule; would be redone). Feeds the later website/web-demo refresh automatically.
- **NEXT:** finish Gate 2 curation (44 left: Sierra/Lapua/Nosler/Barnes/CuttingEdge/Lehigh, same dual-source bar, blanks where uncorroborated) → build Gate 4 UX (+demo) → wire solver into Range&DOPE panel → ship w/ version bump (DISCLAIMER_VERSION already 2). Chad chose to do gates 3&4 review before finishing gate 2; gate 3 done, gate 4 approved.
...
- **Gate-2 IN PROGRESS: 20/61 curated** (Hornady 12 + Berger 5 + Lapua 3, all DUAL-corroborated w/ provenance). **Policy 1 locked** (Chad): maker BC = source of record, cross-check = ANY independent authoritative source (not JBM-only), 2 must agree or BLANK. Permissions FIXED (settings.local.json `ToolName(*)` was silently invalid → bare names; WebFetch/WebSearch now allowed). **Sierra 0/9 = principled blank** (clean Litz G7 obtained but no reliable independent 2nd source here — banded-G1/JS/scrambled-PDF; Litz G7s recorded for fast future fill; #1 fill target). Realistic: Sierra/Cutting Edge/Lehigh/new-Hammer largely principled-blank under Policy 1 until clean 2nd sources; grows data-only post-launch, never blocks ship. **Chad approved adding Hammer Bullets** (do in pre-beta seed-list cleanup pass). NEXT makers: Nosler/Barnes (likely clean JBM Litz+catalog like Berger/Lapua), then Cutting Edge/Lehigh. Proven rigorous pipeline (manufacturer value via WebFetch + JBM b_id cross-check per EXACT bullet; populate only if agree ≤0.006 G7, with `bc_src` provenance + Δ). **Hornady: 7 corroborated** (147/168/178 ELD-M, 225 ELD-M, 143/178 ELD-X, 212 ELD-X — all ≤0.002 vs JBM). `bc_database_version` 0→1. Correctly BLANKED: 6.5 140 ELD-M (Hornady site 0.326 vs JBM 0.305 — revised-BC disagreement; needs Chad tiebreak), A-Tip (ambiguous). Old "no-data tripwire" tests replaced by a structural no-fabrication guard (no BC without `bc_src`). Fuzzy auto-match audit proved bulk automation UNSAFE → every value hand-pinned. **Seed-data bugs found (not BC scope, fix before beta): no 6.5 153gr ELD-M (it's A-Tip); 6mm ELD-M is 108gr not 109gr.** Full method + per-maker worklist: `Ballistic Solver — Gate 2 BC Curation Log.md`.
