...
**✅ PUBLISHED & LIVE 2026-05-17. Chad said "publish" → `website-revamp` merged into `main` (`28765d7`, pushed). GitHub Pages built it (status: built, no error); verified end-to-end on BOTH https://loadscope.app/ and https://chadheidt.github.io/coldbore/ — new scoring FAQ, side-by-side sections, audit fix, and regenerated screenshots all serve HTTP 200. `main`'s CLAUDE.md OBEY-EVERY-TURN work preserved through the merge (only `Notes for next session.md` conflicted; resolved to the current handoff). `website-revamp` branch left intact (merged; don't delete without asking). Scoring FAQ wording mirrors in-app `faq_dialog.py` — keep them in sync if either changes. No app release (pre-beta cadence). ✅ `Loadscope.dmg` rebuilt (signed+notarized v0.15.0, 68.6 MB), uploaded to R2 `coldbore-releases/Loadscope.dmg` 2026-05-17, verified end-to-end (Worker /authorize → signed URL → HTTP 200, exact 68652955 bytes) — that follow-up is CLOSED; the live site fresh-install now serves the current build.**
...
> Continue Loadscope. Read `Notes for next session.md` top section. v0.14.10 (Smart Setup) is shipped. **Ballistic solver GATE 1 IS DONE: Claude drove JBM (standard G7 reference solver) autonomously, it caught + fixed two real bugs (wrong drag table + wrong sight geometry), Loadscope now matches JBM within 0.07 mil on all 4 reference loads — see `Ballistic Solver — Gate 1 Validation RESULT.md`. 185 tests green, JBM oracle is now an automated regression test. Solver STILL not wired/shipped.** Open question for Chad: accept JBM validation as gate-1 sign-off (→ proceed to gates 2/3/4), or also want an optional measured-drag-solver cross-check first? Then gate 2 (authoritative BC DB), gate 3 (disclaimer), gate 4 (predicted-vs-confirmed UX). Hands-off daily cadence (`feedback-hands-off-daily-cadence`).
...
### Ballistic solver — STATUS (2026-05-16: GATE 1 ✅, GATE 3 ✅, GATE 4 UX+demo BUILT (1-4/n), GATE 2 34/75)
- **⚠️ CRASH RECOVERY 2026-05-16 ~11:30 (was using VS Code):** session died with Gate-4 demo work uncommitted in the MAIN working tree. Recovered + committed clean as **`5c56d40` Gate 4 (4/n)** on `main` (local only, NOT pushed). Nothing lost. 35 demo/tour/dope tests green post-recovery; full suite was 206 green.
- **CURRENT (read this first):** Gate 1 ✅ done+signed off. Gate 3 ✅ done (commit 2557422: Chad-approved disclaimer clause #4 + DISCLAIMER_VERSION 1→2 + 4 guard tests; lawyer one-time EULA review flagged in [[loadscope-commerce-flip-on]] Step 0). Solver engine complete: **G7+G1 both authoritative + JBM-validated ≤0.07 mil**. **Gate 4 UX BUILT (commits 40a1d51/82703e1/b563d9c/5c56d40 = 1-4/n):** pure `app/dope_solver.py` overlay module + tests, predicted-vs-confirmed Range&DOPE panel w/ BC/atmo/wind inputs, Pocket Card PREDICTED watermark + pre-range card, AND the solver demo beat (`tools/render_dope_preview.py` → 2 verified screenshots predicted/confirmed + real TOUR_STOPS[3] narration). STILL not wired into the live shell, not shipped. Gate 2 (BC DB) IN PROGRESS 34/75.
- **CORRECTION 2026-05-16 (verify-don't-trust win):** the prior "solver NOT wired into the live shell" claim was WRONG. `main.py:898-900` has wired `DopeEntryPanel` as the live "Range & DOPE" shell panel since **v0.14.9** (`29e65fc`); today's Gate 4 (2/n) upgraded that SAME class in place, so the solver rode in transitively. Verified: 26 solver/panel/shell tests green + `tools/render_dope_preview.py` re-renders the live panel correctly against the demo wb. **Gate 4 = built, LIVE in the app, tested, render-verified. Gate 1/3/4 DONE.**
- **NEXT:** (1) ✅ wiring already done (above). (2) **finish Gate 2 BC curation — IN PROGRESS** (34/75 in `app/resources/component_data.json`, `bc_database_version`=1; 41 blanks; locked dual-source method, ΔG7≤0.010 indep / ≤0.006 copied; blanks are principled & per the curation log NEVER block ship — solver prompts for manual BC, DB grows post-launch via data-only version bumps). Priority order: Sierra match line (Litz G7 already logged) → Hornady 120 ELD-M/BTHP/A-Tip → Berger Hybrids → Barnes/Nosler/Lapua/CE/Lehigh → Hammer 2nd-source. (3) **SHIP = Chad gate** (version bump, DISCLAIMER_VERSION already 2): ship-now-with-principled-blanks vs curate-more-first is HIS product call when curation hits a natural stop. Open Chad item: whether/when to push the `5c56d40`+ main commits.
- **GATE 2 REALITY CHECK 2026-05-16 (don't re-grind this):** ground truth is the JSON (`app/resources/component_data.json` = 34/75, `bc_database_version`=1), NOT the curation-log narrative — the log's per-maker table is STALE (says "Sierra 0/9" but JSON has Sierra 6/9 already done w/ Sierra-G7+xLitz provenance). Of the 41 real blanks: Hammer 14 / Cutting Edge 4 / Lehigh 2 / Lapua Naturalis = ~21 the log already determined STAY principled-blank (not in JBM / single-source / monolithic). Remaining mainstream (Hornady 120 ELD-M, Sierra 150SMK/77TMK/140GK, Berger 7 Hybrids, Barnes 4, Lapua Scenar-L 2, Nosler 140BT) mostly lack a clean independent 2nd source under the no-fabrication policy (e.g. Hornady 120 ELD-M = hornady.com Mach2.25 G7 0.245/G1 0.486 confirmed, but no clean independent corroboration → stays blank per policy). **Net: per-bullet web curation is high-effort/low-yield, the easy fills are already in, and the log itself says blanks NEVER block ship (solver prompts manual BC; DB grows post-launch via data-only `bc_database_version` bump, no app rebuild). Solver is SHIP-READY now at 34/75.**
- **AUTONOMOUS PROGRESS 2026-05-16 PM (Chad offline ~1hr, no hold-gate crossed, all committed local main):** (a) `08496cf` Gate 4 (5/n) — solver now has its OWN demo beat: added a "Confirmed" stop (was unused `dope-preview-2-confirmed.png`), tour is now **7 stops** predicted→confirmed→card, narration flow fixed; headless-verified all stops resolve. (b) `81cf095` Gate 4 (6/n) — the stale `05-pocket-card.png` (v0.14.5, no banner) **regenerated** via new re-runnable `tools/render_predicted_pocket_card.py` from a pre-range workbook so it now shows the **PREDICTED/verify banner + italic rows**; visually verified by Claude. (c) Drafted `RELEASE_NOTES_v0.15.draft.md` (untracked, project root) — customer-facing body in v0.14.10 plain style + the EXACT one-shot ship checklist. (d) Full pytest suite re-run for pre-ship confidence: **206 passed, all green**. **STILL NOT SHIPPED — ship/curate decision still PENDING from Chad (the open boxed question). No version bump, no push.**
- **(historical detail below — gate-1 mechanics, gate-2 method, etc.:)**
- **Gate 1 signed off by Chad (chose "a" = accept JBM validation).** Now in gate 2. **G1 model ADDED (commit 66111c0):** solver was G7-only & refused G1 → blocked BC curation for ~half the seed (Sierra/Nosler/Barnes/older Lapua publish G1). Added authoritative 79-pt standard G1 (JBM mcg1.txt). `solve_trajectory(...,drag_model='G7'|'G1')`; `resolve_g7_bc`→`resolve_bc` returns (bc,model), G1 now resolves (no conversion ever). G1 validated vs JBM ≤0.056 mil; oracle+regression test now cover G7+G1 (6 loads). Suite **187 green**. Still unwired/unshipped.
- **Gate-2 strategy LOCKED by Chad:** manufacturer's own published BC = source of record, JBM library = independent cross-check, curate the 61 seed now, **blanks where uncorroborated** (solver's `resolve_bc` handles blanks safely). Chad also granted "fetch any URL you need."
- **Gate-2 + SEED-LIST CLEANUP DONE.** 34 curated / **75 seed bullets** (9 makers incl. NEW **Hammer** — 14 real bullets from hammerbullets.com WooCommerce API, BCs blank/future-fill: not in JBM). Seed cleanup applied: 9 unambiguous name/weight fixes + 3 phantom→real replacements (Hornady 6.5 153 ELD-M→120 ELD-M; Berger 6.5 156 Hybrid Target→153.5 LRHT; Lapua 6.5 150 Scenar-L→144 Scenar-L) + Nosler 142→140 Custom Competition (g7 0.281). Added 7mm to caliber cascade. Tolerance policy: indep-source ΔG7≤0.010. Remaining blanks principled (JS-only Berger singles, monolithic single-source, Hammer awaiting 2nd source) — grow data-only post-launch, never blocks ship. 191 tests green. Detail: `Ballistic Solver — Gate 2 BC Curation Log.md`.

- **NEXT (gate 2 continues):** remaining ~54 across Berger/Sierra/Lapua/Nosler/Barnes/CuttingEdge/Lehigh, same method, blanks where uncorroborated. Then gates 3 & 4 (drafts staged, await Chad review) → wire into Range&DOPE panel → ship w/ version+disclaimer bump. **Open Chad item: tiebreak Hornady 6.5 140 ELD-M (0.326 vs 0.305).**
- **(prior gate-1 status, still true:)**
- **Path-B locked**: own zero-dep G7 point-mass solver. [[loadscope-ballistic-solver-v015]]
- **GATE 1 EXECUTED AUTONOMOUSLY (commit 045c8be).** Network access works here via `curl`; drove **JBM `jbmtraj-5.1`** (the standard free G7 point-mass reference) for all 4 reference loads. JBM's form: POST, all 62 fields, **unchecked checkboxes must be OMITTED not sent empty** (sending empty = JBM treats as checked → it had silently used point-blank-range zero). JBM has no zero-range field (level scope, Elevation 0.00 MOA); apply 100yd zero analytically `corr = drop − drop[100]*R/100`.
- **Gate 1 caught TWO real bugs the wide-band self-validation had HIDDEN (status-comms lesson):**
  1. **Wrong drag table** — bundled G7 was a hand-truncated 20-pt table with supersonic Cd **5–15% LOW** (M2.0 was 0.2604 vs std 0.2980) → ~8% velocity/TOF/wind error @1000. Replaced with the **authoritative 84-pt standard G7 (McCoy) function published by JBM** (jbmballistics.com/.../text/mcg7.txt — fetch needs full browser UA+Referer or 406). Velocity now within 0.3% of JBM.
  2. **Wrong sight geometry** — zeroing modelled LoS as a flat line sight-height BELOW the bore + bisected a launch angle to it; inflated come-up ~0.5–0.9 mil, load-independently. Replaced with the standard method: fire along the bore, re-zero the straight sight line analytically.
- **RESULT: Loadscope matches JBM within 0.069 mil worst-case** (the .308 transonic @1000; everything supersonic 0.00–0.03 mil) across all 4 loads, all ranges, elevation AND wind. Inside the 0.1 mil gate-1 tolerance. See `Ballistic Solver — Gate 1 Validation RESULT.md`.
- **`tests/jbm_reference.json`** = captured JBM oracle; **`test_matches_jbm_reference_within_gate1_tolerance`** is now a permanent automated regression guard. `_g7_cd` node test updated to authoritative 0.3803/0.2980 (old 0.3884 encoded the bug). Earlier same-session: numerical hardening (exact target interpolation, full point-mass crosswind), BC-DB plumbing (no fabricated values, gate 2 intact). Full suite **185 green**.
- **STILL a safe no-op: nothing imports it (verified). NOT wired, NOT shipped.**
- **GATES 3 & 4 still DRAFTED (unapplied):** `Ballistic Solver — Gates 3 & 4 Review Draft.md` — disclaimer wording + DISCLAIMER_VERSION 1→2 plan, predicted-gray/confirmed-black Range&DOPE UX. Gate 2 (authoritative BC DB) unchanged — `bc_database_version`=0, no values.
- **OPEN for Chad:** accept JBM as gate-1 sign-off (→ proceed gates 2/3/4) OR also want optional measured-drag-solver (4DOF/AB) cross-check (differs by design deep transonic). Then gate 2 → 3 & 4 → wire into Range&DOPE panel + ship w/ version+disclaimer bump.
- **Chad brainstorm logged (answer below in this session):** wants the user-facing explanation of HOW the solver derives its data, to build buyer confidence — folds into [[loadscope-ballistic-solver-v015]] transparency copy + gate-4 UX + marketing.
...
**END-OF-DAY EMAIL — HOW (do not re-derive this; it cost a huge detour 2026-05-16):**
Run `python3 ~/.claude/hooks/send_email.py "Subject" "Body"` (or body on stdin). It POSTs directly to the Resend API as `Loadscope <noreply@loadscope.app>` → `cheidt182@gmail.com`, reading the sending key from `~/.claude/resend.key` (chmod 600, in $HOME, NOT in git). **MUST use curl + a browser User-Agent** (already baked into the script) — this sandbox's default Python urllib UA gets Cloudflare-1010 bot-blocked at api.resend.com. This is "the old way" and is verified working. Do NOT route this through the Cloudflare `coldbore-download` Worker (its repo backup has drifted ahead of live with un-launched Lemon Squeezy commerce code — see the spawned drift-reconciliation task; touching it risks prematurely activating commerce). Resend key source-of-record: `~/Documents/True Zero - v0.12 setup state.md`.
