...
- **Gate-2 FIRST PASS COMPLETE: 24/61 dual-corroborated** (Hornady 12, Berger 5, Lapua 3, Nosler 3, Barnes 1; Sierra/CE/Lehigh 0). 37 PRINCIPLED BLANKS = no-fabrication Policy 1 working (banded-G1/monolithic/small makers, seed-bug/identity mismatches). Every populated BC = 2 independent authoritative sources + `bc_src` provenance; solver prompts user for blanks; DB independently versioned → all 37 fillable post-launch DATA-ONLY, never blocks a ship. Sierra (Litz G7s recorded in curation log) = #1 future fill. Permissions FIXED (settings.local.json bare-names). Hammer approved-to-add in the pre-beta seed-list cleanup (which also fixes phantom 153 ELD-M, 6mm wt, name drift). Full per-maker detail + future-fill list: `Ballistic Solver — Gate 2 BC Curation Log.md`. 191 tests green.
