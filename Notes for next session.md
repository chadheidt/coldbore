...
- **NEXT:** (1) **wire the solver/Gate-4 panel into the live Range&DOPE shell** (the actual hookup — currently the panel + dope_solver exist but nothing in the running app calls them). (2) finish Gate 2 curation (~41 left: Sierra/Lapua/Nosler/Barnes/CuttingEdge/Lehigh, same dual-source bar, blanks where uncorroborated). (3) ship w/ version bump (DISCLAIMER_VERSION already 2). Open Chad item: whether/when to push the `5c56d40` main commit.
