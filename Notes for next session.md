...
- **Gate-2: 33/61 dual-corroborated** (Hornady 12, Sierra 6, Nosler 5, Berger 5, Lapua 4, Barnes 1; CE/Lehigh 0). Tolerance policy: indep-source ΔG7≤0.010 (≤0.006 same-copied), Δ recorded per-bullet. Covers ~all popular match/precision bullets across every major maker. **High-value fills now exhausted**: remaining 28 blanks = JS-only maker sites (Berger single Hybrids), seed-NAME-error identities (need pre-beta seed-list cleanup), genuine single-source monolithics (Barnes/CE/Lehigh). All principled; grow data-only post-launch, never blocks ship. Reusable pipeline proven: maker product-page embedded JSON (Sierra/BigCommerce) + JBM/Litz cross-check. Full detail: `Ballistic Solver — Gate 2 BC Curation Log.md`. 191 tests green.
