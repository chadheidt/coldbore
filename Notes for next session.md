...
- **PENDING CHAD: BC-database sourcing-strategy fork (asked via AskUserQuestion this session).** JBM's 3521-entry library is a queryable authoritative cross-check (has manufacturer + Litz-measured + native-model-flagged entries). The genuine decision is the source-of-record + scope for a PAID product (legal/provenance + timeline + the "500+ bullets" marketing claim). Options + recommendation in this session.
