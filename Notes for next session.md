...
> Continue Loadscope. Read `Notes for next session.md` top section. v0.14.10 (Smart Setup) is shipped. The ballistic solver math is FIXED + numerically hardened + self-validated (184 tests green), still a safe no-op (not wired/shipped). The only thing blocking it now is the 4 Chad-gates. **Gate 1 is ready for you: open `Ballistic Solver — Gate 1 Validation Packet.md`, run the 4 reference loads through the free GeoBallistics web calc (~10 min), fill in the blanks, send back — I diff + iterate the solver, then you review and we do gates 2-4.** Or pick a different priority. Hands-off daily cadence (`feedback-hands-off-daily-cadence`).
...
### Ballistic solver — STATUS (2026-05-16 PM: HARDENED + SELF-VALIDATED, GATE-1 PACKET READY FOR CHAD)
...
- **Retardation math FIXED earlier 2026-05-16** (commit 3e81141): the broken draft *divided* by the retardation coeff instead of *multiplying*; fix derived the standard McCoy/GNU-Ballistics `_RETARD_K = pi*rho0/1152` from first principles inline and applied it correctly. (Detail preserved in the historical section below if needed.)
- **2026-05-16 PM — numerical-hardening pass (commit 2026a01).** Audited the integrator empirically before touching it: forward-Euler is already well-converged at the default dt and the bisection zeroing is tight (~0 MOA at the zero) — **left both alone (didn't over-engineer what was solid)**. Found + fixed ONE real defect: `_fly` snapped each target to the first step *past* it (a sub-yard, dt-dependent overshoot bias); now linear-interpolates exactly to the target → come-up is dt-independent within 0.01 MOA at default dt. Also replaced the closed-form lag-rule crosswind with **full point-mass horizontal drag integration** (agrees with the lag rule to <0.05" through 1000yd, rigorous under heavy wind / transonic / off-std BC).
- **BC-DB plumbing built (NO fabricated values — gate 2 respected):** `bc_database_version` in component_data.json (=0: none curated yet), `component_data.bullet_bc()` native g7/g1 reader, `ballistics.resolve_g7_bc()` with `BcUnavailable`/`BcModelUnsupported` (refuses to convert G1, never guesses). Authoritative values still Chad's gate 2.
- **Self-validation harness**: `tests/test_ballistics.py` **10 → 20**, full suite **174 → 184 green**. Added dt-independence, tight-zero, integrated-crosswind-vs-lag-rule, BC version/lookup/override/refusal guards + a tripwire test that FAILS if fabricated BCs are ever added. Reference 6.5 CM 140 ELD-M (2710/0.315/100yd) → **12.30 MOA @500, 31.56 MOA / 9.18 mil @1000, TOF 1.446s, 59.7" drift @1000** (full point-mass crosswind) — squarely in the published regime. Self-check is necessary-but-NOT-sufficient.
- **STILL a safe no-op: nothing imports it (verified). NOT wired, NOT shipped.**
- **GATE-1 PACKET STAGED FOR CHAD:** `Ballistic Solver — Gate 1 Validation Packet.md` — 4 reference loads (6.5CM 140 ELD-M, .308 175 SMK, .300 PRC 225 ELD-M, 6 ARC 108 ELD-M) with exact inputs + solver outputs + blanks for the pro-solver readback; recommends free GeoBallistics web calc; ~10 min Chad task. Live pro-solver query is NOT automatable here (JBM = POST CGI; 4DOF/GeoBallistics/AB = JS SPAs/paid) — confirmed honestly, not faked.
- **GATES 3 & 4 DRAFTED FOR CHAD:** `Ballistic Solver — Gates 3 & 4 Review Draft.md` — exact disclaimer wording + DISCLAIMER_VERSION 1→2 plan (NOT applied), and the predicted(gray)-vs-confirmed(black) Range&DOPE UX design + card watermark + tour-narration rewrite (NO code written). One coherent review, post-gate-1.
- **Next on the solver:** Chad runs the gate-1 packet → Claude diffs solver vs pro-solver, iterates if outside 0.1 mil / 0.5 MOA @1000 → Chad reviews → gates 2 (authoritative BC data) → 3 & 4 (approve drafts) → wire into Range&DOPE panel + ship with version/disclaimer bump.
