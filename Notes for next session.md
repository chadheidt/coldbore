...
### Ballistic solver — STATUS (2026-05-16: MATH FIXED + SELF-VALIDATED, STILL SHIP-GATED)
...
- **2026-05-16 — retardation math FIXED.** Root cause of the broken draft: the integrator *divided* by the retardation coefficient instead of *multiplying* (`decel = sigma*cd*v / (g7_bc*K)`), a ~1/K² ≈ 2.3e7 error = the "~7 orders" the sanity test caught. The scaffold's K *value* was essentially right; the bug was the operator/structure. Fix: derived the standard McCoy/GNU-Ballistics G7-BC point-mass coefficient from first principles inline (`_RETARD_K = pi*rho0/1152`, with the d² and g cancellations documented in-code) and applied it correctly. Drag table, `air_density_ratio()`, module API were already correct.
- **Self-validation harness added**: `tests/test_ballistics.py` (10 tests, all green; full suite **174 pass**). Tier 1 = hard physical invariants (≈0 come-up at zero, monotonic come-up, plausible+increasing TOF, growing crosswind drift, finite outputs). Tier 2 = published-reference ballpark for the 6.5 CM 140 ELD-M load (2710 fps, G7 0.315, 100yd zero) → solver gives 12.3 MOA @500, **31.6 MOA / 9.18 mil @1000, TOF 1.45s, 59.6" drift @1000 (10mph full-value)** — squarely in the published regime.
- **STILL a safe no-op: nothing imports it. NOT wired, NOT shipped.** The self-check is necessary-but-NOT-sufficient (tier-2 references are from training knowledge, not a live pro-solver query).
- **REMAINING ship-gates (require Chad — do NOT bypass):** (1) validate vs a LIVE Hornady 4DOF / GeoBallistics / Applied Ballistics run on the reference loads + Chad review; (2) BC database = authoritative manufacturer-sourced G7 values, do NOT fabricate (`component_data.py` has the maker-organized bullet/primer SEED, no BC values yet); (3) signed-disclaimer predicted-DOPE language + `DISCLAIMER_VERSION` bump (re-prompt all users); (4) predicted-vs-confirmed DOPE UX in the Range&DOPE panel + Chad review.
- **Next on the solver when Chad's ready:** run gate (1) — Chad pulls the reference-load DOPE from a live pro solver, Claude diffs solver output vs it, iterate if off, Chad-review → only then gates 2-4 + wiring into the Range&DOPE panel.
