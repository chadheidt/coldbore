...
4. Once `dist/Loadscope.app` exists, zip via `ditto -c -k --keepParent "Loadscope.app" "Loadscope.zip"`, then add the Quick Start docx via `zip -j "Loadscope.zip" "../Loadscope — Quick Start.docx"`.
5. `gh release create vX.Y.Z --repo chadheidt/coldbore --title "Loadscope X.Y.Z" --notes-file <notes> --draft "dist/Loadscope.zip"`.
...
7. Chad opens Loadscope from `/Applications` → yellow banner → Install Update → Quit and Install → app reopens at new version. Tools → About reports new version.
